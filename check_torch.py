...
from protes_torch import protes_torch
...
def calc_control(d_list=[25, 50, 100], M=int(1.E+4), constr=False):
    ...
    log = Log(f'result/logs_check_torch/control{"_constr" if constr else ""}.txt')
    ...
    for d in d_list:
        ...
        t = tpc()
        n_opt, y_opt = protes_torch(f_batch, [2]*d, M, constr=constr, log=True)
        t = tpc() - t

        text = ''
        text += f' ODE-{d}D | '
        text += f'TOR v={y_opt:-9.2e}, t={t:-9.2e} | '
        log(text)
...
def calc_func(d=7, n=16, M=1.E+4, M_ng=1.E+4, with_shift=True):
    ...
    nm = 'FUNC'
    log = Log(f'result/logs_check_torch/func.txt')
    ...
    for func in teneva.func_demo_all(d, names=names, with_piston=True):
        ...
        t = tpc()
        n_opt, y_opt = protes_torch(f, [n]*d, M, log=True)
        t = tpc() - t
...
def calc_knapsack(M=1.E+4, M_ng=1.E+4):
    ...
    nm = 'KNAPSACK'
    log = Log(f'result/logs_check_torch/knapsack.txt')
    ...
    for task in [1]:
        ...
        t = tpc()
        n_opt, y_opt = protes_torch(f_batch, [2]*d, M, log=True)
        t = tpc() - t
...
def calc_qubo(d=50, M=1.E+4, M_ng=1.E+4):
    ...
    nm = 'QUBO'
    log = Log(f'result/logs_check_torch/qubo.txt')
...
if __name__ == '__main__':
    ...
    folder_ensure('result/logs_check_torch')
